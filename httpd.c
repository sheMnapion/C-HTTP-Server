...
Note that the dir must start with .";
char *mapSuffix[]={"php","jpg","jpeg","png","ico","gif","html","css","js","json","pdf","pptx"};
char *mapTypes[]={"application/x-php","image/jpg","image/jpeg","image/png","image/ico","image/gif","text/html","text/css","application/js","application/json","application/pdf","application/x-ppt"};
...
void analyzeState(int argc,char *argv[])
{
	...
	if(portGiven==false||sitePathGiven==false)
		programState=0;
...
//	printf("Buffer:\n%s\n",buffer);
	...
	printf("Path:%s\n",getStr);
	if(strcmp(getStr,"/")==0)
		getStr=strdup("/index.html");
	getStr++;
	struct stat stateBuf;
	int ac=stat(getStr,&stateBuf);
	...
	if(ac<0){
		...
		if(S_ISDIR(stateBuf.st_mode)){
			...
			if(ac<0){
				perror("access index.html");
				accessAble=false;
			}
		}
		else if(S_ISREG(stateBuf.st_mode)){
			printf("Regular file!\n");
		}
		else{
	...
	printf("State mode:%d\n",stateBuf.st_mode);
	volatile int fileLength=0;
	volatile int fileFd=-1;
	pthread_mutex_lock(&gzipLock);
	if(accessAble){
		printf("File:%s\n",getStr);
	...
	if(wc!=headerLength+fileLength){
		printf("Wrote %d bytes while true one is %d(%d+%d)\n",wc,headerLength+fileLength,headerLength,fileLength);
		printf("\033[31mFor %d, fd%d\n\033[0m",rg->requestTime,fd);
...
void mainDealer()
{
	...
	pthread_mutex_init(&gzipLock,NULL);
